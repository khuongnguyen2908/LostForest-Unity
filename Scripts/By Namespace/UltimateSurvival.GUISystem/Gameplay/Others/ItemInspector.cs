...
namespace UltimateSurvival.GUISystem
{
	public class ItemInspector : GUIBehaviour
	{
		...
		private void On_ConsumeClicked()
		{
            ...
            bool canRefill = item.HasProperty("Can Consume") &&
                item.HasProperty("Consume Amount") &&
                item.HasProperty("Container Amount") &&
                item.HasProperty("container Capacity");

            if(canRefill && item.GetPropertyValue("Container Amount").Float.Current <= 0f)
            {
                MessageDisplayer.Instance.PushMessage(
                               string.Format("<color=red>{0}</color> is empty", item.Name)
                           );
                return;
            }
            ...
            if (item.HasProperty("Can Consume") && Time.time - m_LastConsumeTime > 3f)
			{
                ...
                if (canRefill)
                {
                    if (item.GetPropertyValue("Container Amount").Float.Current > 0f)
                    {
                        float ConsumeAmount = item.GetPropertyValue("Consume Amount").Float.Current;
                        float RefillMax = item.GetPropertyValue("Container Capacity").Float.Current;
                        var CurrentAmount = item.GetPropertyValue("Container Amount").Float;

                        CurrentAmount.Current -= ConsumeAmount;
                        CurrentAmount.Current = Mathf.Clamp((Mathf.Round(CurrentAmount.Current * 10f) / 10f), 0.0f, RefillMax);

                        item.GetPropertyValue("Container Amount").SetValue(ItemProperty.Type.Float, CurrentAmount);
